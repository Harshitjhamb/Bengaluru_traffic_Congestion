...
| AUC 0.98 | Priority classifier's real, measured test-set ROC-AUC is **0.9402** (`models/v2_metrics.json`). This is the one resume number worth knowing precisely before an interview: the model was deliberately regularized (see below) to fix a genuine overconfidence problem, which cost some raw AUC. Closure's AUC is much lower (0.7724 — a harder, imbalanced problem) and duration is regression (no AUC). If asked "AUC of what, exactly," the precise answer is "the priority classifier, and the honest number is 0.94, not 0.98." | ⚠️ resume rounds up — know the real number (0.94) and why |
| 89% avg confidence | Real measured average (`mean(max(p, 1-p))` across the priority + closure test sets) is **80.0%**. This dropped from an earlier build's 88.4% on purpose — that earlier version was returning 96%+ confidence on the large majority of inputs (including ones with no real historical precedent), which is a red flag for a live demo, not a strength. | ⚠️ resume overstates by ~8 points — know why, and be ready to explain it as a deliberate trade-off |
...
>
> The priority classifier is the standout result — ROC-AUC 0.94, Accuracy 88%. I validated the underlying pattern with a spatial holdout — trained on one set of GPS locations, tested on a completely disjoint set the model had never seen — and accuracy held at 88%, AUC at 0.93, so it's genuinely learning a spatial+temporal pattern, not memorizing repeated incident locations. An earlier version of this model actually scored higher, AUC 0.99, but I deliberately walked that back in two rounds: first for being overconfident in aggregate, then again after noticing that even with GPS held fixed, confidence barely moved across different causes — meaning the model was really only listening to location. Forcing individual trees to sometimes train without access to GPS columns (`colsample_bytree`/`colsample_bylevel`) fixed that. Regularizing it down cost real AUC but bought a model whose confidence you can actually trust when it's high or take seriously when it's not.
>
...
- **Model 1 — Priority classifier:** `XGBClassifier(n_estimators=150, max_depth=3, learning_rate=0.1, reg_lambda=5, min_child_weight=10, colsample_bytree=0.5, colsample_bylevel=0.5, scale_pos_weight=neg/pos)`. Features deliberately **exclude `corridor`** — it correlates with priority almost 1:1 in some slices of the data, a leakage risk. Trained on 8,006 rows (6,404/1,602 stratified split). **Result: Accuracy 0.8833, F1 0.9047, ROC-AUC 0.9402.** None of these hyperparameters are defaults — see Part 5, Concept 9 for why each one was added, in two separate rounds: the shallow depth and `reg_lambda`/`min_child_weight` fixed overall overconfidence (an earlier version hit AUC 0.9924 but 96%+ confidence almost everywhere); the `colsample_bytree`/`colsample_bylevel` dampening was a *second*, later fix for a more specific problem — even after the first round, confidence stayed pinned around 90% for *any* cause or time of day once GPS coordinates were held fixed, because the model still leaned almost entirely on location. Randomly hiding location columns from individual trees forces cause/time to actually matter.
...
- **Avg confidence:** `mean(max(p, 1-p))` across the priority and closure test-set predictions = **80.0%** — the real, computed number behind the resume's "89% avg confidence" (a meaningful gap, worth knowing precisely — see Part 5, Concept 9 for why the true number is lower than the resume states, on purpose).
...
### 9. A legitimate result can still be a badly-behaved one — fixing model overconfidence
**What we found:** Even after confirming (Concept 8) that the priority model's strong performance wasn't leakage, actually using the live app with varied, sometimes-arbitrary inputs showed a different problem: the model returned 96%+ confidence on the large majority of predictions — for random GPS coordinates, unusual cause/time combinations, essentially anything. Real accuracy doesn't mean every individual prediction *deserves* that much certainty.
**Why this happens:** With deep-enough trees (`max_depth=6`, 300 estimators) and highly separable spatial features (hotspot distances effectively encode a fine-grained location), XGBoost can partition the feature space into very small, very high-purity leaf regions. Any query landing in one of those regions gets pushed toward 0 or 1 — which is *locally* accurate (the spatial-holdout check in Concept 8 confirms that) but produces a confidence signal that stops being informative, because it's extreme almost everywhere, not just where the evidence is genuinely strong.
**What we did about it — round 1:** Reduced `max_depth` from 6 to 3, and added `reg_lambda=5` and `min_child_weight=10` to discourage tiny, ultra-high-purity leaves. On synthetic random-coordinate queries spanning Bengaluru, this dropped the fraction with confidence ≥96% substantially. But testing the actual deployed form (default GPS coordinates pre-filled, varying only cause and time — the way a real user clicking through the demo behaves) exposed a second, more specific problem.
**Round 2 — the fixed-location finding:** With GPS held constant at the form's default and only cause/hour changed, confidence stayed pinned around 90% regardless — `vip_movement`, `accident`, `pot_holes`, `test_demo` all produced nearly identical confidence at the same location, because the model was still leaning almost entirely on latitude/longitude/hotspot-distance features. Cause and time-of-day were mathematically present in the feature vector but functionally close to ignored for that fixed location.
**What fixed it:** Added `colsample_bytree=0.5` and `colsample_bylevel=0.5`, which force each tree (and each split within a tree) to randomly ignore half of the available columns. Concretely, this means a meaningful fraction of the 150 trees never see latitude/longitude/hotspot-distance at all for some of their splits, and have to find signal in cause/time/zone instead. At the same fixed default location, confidence across the 16 possible causes now ranges from ~72% to ~87% instead of sitting flat at ~90%+.
**The cost, honestly stated:** Test-set AUC moved from 0.9519 (round 1) to 0.9402 (round 2), accuracy from 90.2% to 88.3% — a further, deliberate step down from the original 0.9924/96.9%, still comfortably above the pre-XGBoost RandomForest-era baseline (AUC not computed, accuracy 76.97%).
**Why this two-round process matters more than either number alone:** Round 1 fixed "the model is too sure everywhere." Round 2 fixed a subtler, easy-to-miss failure: a model can look well-calibrated in aggregate (reasonable spread across many different random inputs) while still being *functionally insensitive to most of its own features* for any single fixed input. Only testing the actual demo flow — not just synthetic random sampling — surfaced that.
**Interview one-liner:** "The first regularization pass fixed overconfidence in aggregate, but testing the actual demo — same location, different causes — showed confidence barely moved at all, meaning the model was really only listening to GPS. Forcing trees to sometimes train blind to location, via column subsampling, is what actually made cause and time of day matter."
...
#### Q4b. Your resume says AUC 0.98, but you just said 0.94 — which is it?
...
**Strong answer:** "0.94 is the honest, current number, and I know exactly why it's lower than what's on my resume: an earlier version of this model hit AUC 0.99, but I found it was returning 96%+ confidence on almost every prediction — first in aggregate across random inputs, and then, even after a first fix, still nearly flat regardless of cause once I held GPS coordinates fixed, meaning it was really only listening to location. I regularized it down in two rounds, which together cost about 0.05 of AUC, to get a model whose confidence is actually meaningful and actually responds to the incident details, not just where it happened. I'd rather explain that trade-off than quietly keep the inflated number." Walk through Part 5, Concept 9 if asked for detail.
...
**How to defend it:** Point to `notebook/train_v2.py` for the full pipeline and `models/v2_metrics.json` for the exact numbers. Precisely: priority ROC-AUC is **0.9402**, not 0.98 — know this gap and why it exists (Part 5, Concept 9: an unregularized version hit 0.99 but was badly overconfident; the deployed model trades some AUC for realistic, varied confidence). Closure AUC is 0.7724 (a much harder, imbalanced problem — be ready to name this distinction if asked "AUC of what"). Avg confidence is **80.0%**, meaningfully below the resume's 89% — again, on purpose. Be ready to also state the honest counter-numbers: closure F1 0.3694 and duration R² 0.2193 are both modest in absolute terms.
...
| Biggest result? | Priority classifier: ROC-AUC 0.9402, validated with a spatial holdout (AUC 0.9295) to rule out location-memorization. |
| Real number vs resume number? | Resume says AUC 0.98 / 89% confidence; real, current numbers are AUC 0.9402 / 80.0% confidence — deliberately lower after fixing an overconfidence problem (Concept 9). |
...
| Most important validation habit? | Not trusting a suspiciously good metric, twice over — checked it wasn't leakage (spatial holdout), then checked it wasn't overconfidence dressed up as accuracy (regularized the model down from AUC 0.99 to 0.94 in two rounds after finding it was almost only listening to GPS location). |
