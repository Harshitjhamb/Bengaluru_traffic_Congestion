...
- **Model 1 — Priority classifier:** `XGBClassifier(n_estimators=150, max_depth=3, learning_rate=0.1, reg_lambda=5, min_child_weight=10, scale_pos_weight=neg/pos)`. Features deliberately **exclude `corridor`** — it correlates with priority almost 1:1 in some slices of the data, a leakage risk. Trained on 8,006 rows (6,404/1,602 stratified split). **Result: Accuracy 0.9020, F1 0.9205, ROC-AUC 0.9519.** The shallow depth and regularization (`reg_lambda`, `min_child_weight`) are deliberate, not default choices — an earlier, deeper/unregularized version of this exact model reached AUC 0.9924 but returned 96%+ confidence on the large majority of predictions (including for essentially arbitrary GPS coordinates), which is overfitting to fine-grained spatial clusters, not genuine certainty. See Part 5, Concept 9.
...
- **Avg confidence:** `mean(max(p, 1-p))` across the priority and closure test-set predictions = **80.8%** — the real, computed number behind the resume's "89% avg confidence" (a meaningful gap, worth knowing precisely — see Part 5, Concept 9 for why the true number is lower than the resume states, on purpose).
...
**What we did about it:** Reduced `max_depth` from 6 to 3, and added `reg_lambda=5` and `min_child_weight=10` to discourage tiny, ultra-high-purity leaves. On 200 synthetic random-coordinate queries spanning Bengaluru, this dropped the fraction with confidence ≥96% from roughly two-thirds to about one-third, with the median moving from ~99% to ~81% and a real spread from the mid-50s to high-90s. The cost: test-set AUC fell from 0.9924 to 0.9519, and accuracy from 96.9% to 90.2% — both still comfortably above the pre-XGBoost RandomForest-era baseline, just less inflated.
**Why this is worth doing even though it costs headline metrics:** A model that's always 96%+ confident is not more trustworthy than one that says 65% sometimes and 95% other times — it's less trustworthy, because it's stopped distinguishing genuinely clear-cut incidents from ambiguous ones. For a system feeding a human dispatcher's decision-making, a confidence number that means something is more valuable than a slightly higher AUC.
**Interview one-liner:** "Getting AUC 0.99 was easy — the harder, more valuable engineering call was noticing that number came with the model being *always* extremely confident, deciding that wasn't actually a good property for a decision-support tool, and giving up a few points of AUC to fix it."
...
**Strong answer:** "0.95 is the honest, current number, and I know exactly why it's lower than what's on my resume: an earlier version of this model hit AUC 0.99, but I found it was returning 96%+ confidence on almost every prediction, including ones with no real basis for that much certainty — a classic sign of a model memorizing fine-grained spatial patterns rather than expressing calibrated uncertainty. I regularized it down on purpose, which cost about 0.04 of AUC, to get a model whose confidence is actually meaningful. I'd rather explain that trade-off than quietly keep the inflated number." Walk through Part 5, Concept 9 if asked for detail.
...
**How to defend it:** Point to `notebook/train_v2.py` for the full pipeline and `models/v2_metrics.json` for the exact numbers. Precisely: priority ROC-AUC is **0.9519**, not 0.98 — know this gap and why it exists (Part 5, Concept 9: an unregularized version hit 0.99 but was badly overconfident; the deployed model trades some AUC for realistic, varied confidence). Closure AUC is 0.7724 (a much harder, imbalanced problem — be ready to name this distinction if asked "AUC of what"). Avg confidence is **80.8%**, meaningfully below the resume's 89% — again, on purpose. Be ready to also state the honest counter-numbers: closure F1 0.3694 and duration R² 0.2193 are both modest in absolute terms.
...
| Most important validation habit? | Not trusting a suspiciously good metric twice over — once to check it wasn't leakage (spatial holdout), and again to check it wasn't just overconfidence dressed up as accuracy (regularized the model down from AUC 0.99 to 0.95 once I saw it return 96%+ confidence almost everywhere). |
