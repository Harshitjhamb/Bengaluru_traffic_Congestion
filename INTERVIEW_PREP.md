...
| AUC 0.98 | Priority classifier's real, measured test-set ROC-AUC is **0.9511** (`models/v2_metrics.json`). This is the one resume number worth knowing precisely before an interview: the model was deliberately regularized (see below) to fix a genuine overconfidence problem, which cost some raw AUC. Closure's AUC is much lower (0.7815 — a harder, imbalanced problem) and duration is regression (no AUC). If asked "AUC of what, exactly," the precise answer is "the priority classifier, and the honest number is 0.95, not 0.98." | ⚠️ resume rounds up — know the real number (0.95) and why |
| 89% avg confidence | Real measured average (`mean(max(p, 1-p))` across the priority + closure test sets) is **81.0%**. This dropped from an earlier build's 88.4% on purpose — that earlier version was returning 96%+ confidence on the large majority of inputs (including ones with no real historical precedent), which is a red flag for a live demo, not a strength. | ⚠️ resume overstates by ~8 points — know why, and be ready to explain it as a deliberate trade-off |
...
9. `closure_model.predict_proba()` (XGBoost) gives P(needs road closure), thresholded at the **tuned** value stored in the bundle (`closure_threshold ≈ 0.625`), not the default 0.5.
...
- **Model 1 — Priority classifier:** `XGBClassifier(n_estimators=150, max_depth=3, learning_rate=0.1, reg_lambda=1, min_child_weight=3, colsample_bytree=0.6, colsample_bylevel=0.6, scale_pos_weight=neg/pos)`, trained on features that now include `cause_score` (the numeric per-cause severity weight) as its own direct feature, not just gated inside interaction terms. Features deliberately **exclude `corridor`** — it correlates with priority almost 1:1 in some slices of the data, a leakage risk. Trained on 8,006 rows (6,404/1,602 stratified split). **Result: Accuracy 0.8939, F1 0.9144, ROC-AUC 0.9511.** None of these choices are defaults — see Part 5, Concept 9 for why each was added, across three separate rounds of testing the actual live demo, not just aggregate metrics: shallow depth + `reg_lambda`/`min_child_weight` fixed overall overconfidence; `colsample_bytree`/`colsample_bylevel` fixed the model leaning almost entirely on location; adding `cause_score` directly fixed cause signal being structurally invisible outside peak hours.
- **Model 2 — Road closure classifier:** `XGBClassifier(n_estimators=300, max_depth=4, learning_rate=0.05, scale_pos_weight=(neg/pos)×0.6)` on 7,987 rows, 7.26% positive class. The hyperparameters (shallower trees, dampened `scale_pos_weight`) came from a small grid search over depth/estimators/learning-rate/imbalance-dampening, selected by F1 after threshold tuning — undampened `scale_pos_weight` and deeper trees over-predicted the minority class and scored worse. Threshold tuned via `precision_recall_curve` to **0.625** (not the default 0.5). **Result: Accuracy 0.9043, F1 0.3651, ROC-AUC 0.7815.**
...
- **Avg confidence:** `mean(max(p, 1-p))` across the priority and closure test-set predictions = **81.0%** — the real, computed number behind the resume's "89% avg confidence" (a meaningful gap, worth knowing precisely — see Part 5, Concept 9 for why the true number is lower than the resume states, on purpose).
...
- Also present: `priority_feature_cols` (60 cols — up from a pre-v2 baseline of 47, after adding hotspot distances, circular time, interactions, and `cause_score`), `closure_feature_cols` (82 cols), `duration_feature_cols` (79 cols), `closure_threshold` (0.625), `cat_features`/`num_features` (+ `_fixed` variants), `cause_score_map`, `manpower_map`, `zone_station_map` (10 zones), `station_coords` (8,120 rows), `corridor_coords` (8,101 rows).
...
**How it works in this project:** `precision_recall_curve` + `f1_scores = 2*(p*r)/(p+r+1e-9)`; `best_threshold = thresholds[f1_scores.argmax()]` → **0.625**, stored in the bundle and used at inference (`prob_closure >= B['closure_threshold']`).
...
### 8. Validating a suspiciously good result isn't leakage
**What we found:** An early priority-classifier run — 97% accuracy, AUC 0.99 — was high enough for a real-world tabular problem that it deserved active suspicion, not just acceptance.
**What we checked:** First, whether `cause` or `zone` alone near-deterministically implies `priority` (it doesn't — priority rates by cause range 32%–69%, no near-100% pattern; by zone, 30%–82%, also no perfect determinism). Second, whether the 80/20 *random* split let many test-set incidents share an almost-identical GPS location with a training-set incident carrying the same priority label — a real risk, since real-world incident data repeats: only 3,336 unique ~100m GPS clusters exist across 8,173 rows, and individual clusters can be 100% one class (e.g., one 65-incident cluster is 100% "High," another 53-incident cluster is 100% "Low"). To rule this out, I re-evaluated with a **spatial group-holdout** (`GroupShuffleSplit` grouped by rounded lat/lon, so no test-set location's ~100m cluster appears in training) instead of a random split. With the deployed, regularized model, spatial-holdout Accuracy is 0.8852 / AUC 0.9479 — close to the random-split numbers (Accuracy 0.8939 / AUC 0.9511), and the same pattern held with the earlier, deeper model too (spatial-holdout numbers barely moved from its random-split ones either).
**What this means:** The model is genuinely learning a spatial+temporal decision boundary that generalizes to locations it never saw in training, not memorizing repeated addresses — the strong performance is real signal, not a train/test leak. That's a materially stronger claim than "our test accuracy is high," and it directly pre-empts the most obvious follow-up question a good interviewer would ask about *whether the number is legitimate at all*. (Concept 9, next, is the separate, follow-up question of whether a legitimate number is nonetheless a *well-behaved* one.)
**Interview one-liner:** "A number that good is exactly what should make you suspicious of your own pipeline before you're proud of it — so before trusting it, I re-validated with a spatial holdout where the test locations never appeared in training at all, and the result held. That's what told me the signal was real."

### 9. A legitimate result can still be a badly-behaved one — fixing model overconfidence, in three rounds
**What we found:** Even after confirming (Concept 8) that the priority model's strong performance wasn't leakage, actually using the live app with varied, sometimes-arbitrary inputs showed a different problem: the model returned 96%+ confidence on the large majority of predictions. Real accuracy doesn't mean every individual prediction *deserves* that much certainty.
**Why this happens:** With deep-enough trees (`max_depth=6`, 300 estimators) and highly separable spatial features (hotspot distances effectively encode a fine-grained location), XGBoost can partition the feature space into very small, very high-purity leaf regions. Any query landing in one of those regions gets pushed toward 0 or 1 — locally accurate, but a confidence signal that's stopped being informative because it's extreme almost everywhere.

**Round 1 — fix aggregate overconfidence.** Reduced `max_depth` from 6 to 3, added `reg_lambda` and `min_child_weight` to discourage tiny, ultra-high-purity leaves. On synthetic random-coordinate queries spanning Bengaluru, this dropped the fraction of predictions with confidence ≥96% substantially (AUC 0.9924→0.9519, accuracy 96.9%→90.2%). But testing the actual deployed form — default GPS pre-filled, only cause/hour changed, the way someone actually clicks through the demo — exposed a second, more specific problem.

**Round 2 — fix location dominance.** With GPS held constant at the form's default, confidence stayed pinned around 90% regardless of cause — the model was still leaning almost entirely on latitude/longitude/hotspot-distance features. Added `colsample_bytree=0.6`/`colsample_bylevel=0.6`, forcing individual trees (and splits within them) to randomly train without access to a fraction of columns, including sometimes all the location ones — forcing cause/time to matter somewhere. This helped (confidence at the fixed location dropped out of the 90s) but revealed a *third* problem when I checked how much it varied across different causes specifically.

**Round 3 — fix a missing feature, not just a hyperparameter.** At the fixed location, most of the 16 causes were still producing nearly identical confidence (rounding to the same 1-2 percentage points), even with location de-emphasized. The reason: `cause_score` — the hand-authored numeric severity weight per cause (0–5) — was never given to the model as its own feature. It only ever appeared *inside* `peak_x_cause`/`weekend_x_cause`, both of which are exactly **zero** whenever `is_peak_hour` and `is_weekend` are both 0 — i.e., cause severity was invisible to the model outside peak hours entirely, and even during peak hours it was entangled with a time flag rather than standing on its own. Adding `cause_score` directly to the feature set (this is not leakage — it depends only on the cause itself, not on any target label) let the model use it unconditionally. At the same fixed default location, confidence across the 16 causes now spans **~73%–85%** (7 distinct values) — matching the fact that `cause_score_map` itself only defines 6 real severity tiers, which is the right amount of differentiation to expect, not an arbitrary ceiling.

**The cost, honestly stated, across all three rounds:** AUC went 0.9924 → 0.9519 (round 1) → 0.9402 (round 2) → 0.9511 (round 3, since the new feature recovered some signal); accuracy 96.9% → 90.2% → 88.3% → 89.4%. Final numbers are comfortably above the pre-XGBoost RandomForest baseline (76.97% accuracy, AUC not computed) and only slightly below the original resume-matching numbers, for a model that now actually responds to the incident details instead of just the pin on the map.

**Why this three-round process matters more than any single number:** Each round fixed a different, easy-to-miss failure mode that only surfaced by testing the *actual demo interaction pattern*, not synthetic random sampling: (1) the model was too sure everywhere, (2) it was only listening to one feature group, (3) an entire engineered feature was structurally invisible to it most of the time. None of these would show up in a single aggregate metric like "test AUC."
**Interview one-liner:** "Getting a model to perform well is round one. Getting it to actually explain its confidence in terms a dispatcher could trust took two more rounds of specifically testing the demo the way a real user would — same location, different causes — and each round surfaced a different reason the model wasn't really using the features I'd built for it."
...
**Strong answer:** Walk through Part 5, Concept 8 — the cause/zone determinism check, the GPS-cluster-repetition finding, and the spatial group-holdout result. This is one of the two best-prepared answers in the whole document; don't rush it.

#### Q4b. Your resume says AUC 0.98, but you just said 0.95 — which is it?
**What the interviewer is asking:** Will you own a discrepancy between what's written and what you actually did, or get flustered?
**Strong answer:** "0.95 is the honest, current number, and I know exactly why it's lower than what's on my resume: an earlier version of this model hit AUC 0.99, but I found it was returning 96%+ confidence on almost every prediction — first in aggregate across random inputs, then still nearly flat regardless of cause once I held GPS coordinates fixed (meaning it was really only listening to location), and then I found a specific engineered feature, cause severity, was structurally invisible to the model outside peak hours due to how I'd gated it into an interaction term. I fixed all three, in three separate rounds, which together cost a few points of AUC, to get a model whose confidence is actually meaningful and actually responds to the incident details, not just where it happened. I'd rather explain that trade-off than quietly keep the inflated number." Walk through Part 5, Concept 9 if asked for detail.
...
**How to defend it:** Point to `notebook/train_v2.py` for the full pipeline and `models/v2_metrics.json` for the exact numbers. Precisely: priority ROC-AUC is **0.9511**, not 0.98 — know this gap and why it exists (Part 5, Concept 9: an unregularized version hit 0.99 but was badly overconfident; the deployed model trades some AUC for realistic, varied confidence). Closure AUC is 0.7815 (a much harder, imbalanced problem — be ready to name this distinction if asked "AUC of what"). Avg confidence is **81.0%**, meaningfully below the resume's 89% — again, on purpose. Be ready to also state the honest counter-numbers: closure F1 0.3651 and duration R² 0.2193 are both modest in absolute terms.
...
| Biggest result? | Priority classifier: ROC-AUC 0.9511, validated with a spatial holdout (AUC 0.9479) to rule out location-memorization. |
| Real number vs resume number? | Resume says AUC 0.98 / 89% confidence; real, current numbers are AUC 0.9511 / 81.0% confidence — deliberately lower after fixing an overconfidence problem (Concept 9). |
...
| Most important validation habit? | Not trusting a suspiciously good metric — checked it wasn't leakage (spatial holdout), then checked it wasn't overconfidence dressed up as accuracy in two more rounds (found it was almost only listening to GPS, then found a whole feature was structurally invisible outside peak hours). |
| Biggest limitation? | Closure F1 (0.3651) is still weak in absolute terms — the closure task is fundamentally data-starved (580 positive examples), not something feature engineering alone fixes. |
...
| How does road-closure prediction work? | XGBoost's probability compared against a tuned threshold (0.625), picked by maximizing F1 on a precision-recall sweep, with `scale_pos_weight` dampened to ×0.6 via grid search. |
