...
def load_bundle():
    import joblib
    return joblib.load('models/recommendation_engine_bundle_v2.pkl')
...
def add_hotspot_distances(event):
    for name, (hlat, hlon) in HOTSPOTS.items():
        event[f'dist_{name}_km'] = haversine_km(event['latitude'], event['longitude'], hlat, hlon)
    return event


def add_circular_time_features(event):
    event['hour_sin'] = np.sin(2 * np.pi * event['hour'] / 24)
    event['hour_cos'] = np.cos(2 * np.pi * event['hour'] / 24)
    event['month_sin'] = np.sin(2 * np.pi * event['month_num'] / 12)
    event['month_cos'] = np.cos(2 * np.pi * event['month_num'] / 12)
    event['dow_sin'] = np.sin(2 * np.pi * event['day_of_week'] / 7)
    event['dow_cos'] = np.cos(2 * np.pi * event['day_of_week'] / 7)
    return event
...
def recommend_resources(event):
    ...
    event = add_hotspot_distances(event)
    event = add_circular_time_features(event)
    event = add_interaction_features(event)
    ...
    # 2. Road closure (XGBoost, tuned threshold)
    ...
    # 3. Estimated severity — from PREDICTIONS, not ground truth (no leakage)
    cause_w = B['cause_score_map'].get(event['event_cause'], 1)
    ...
    # 4. Duration (XGBoost, log1p target)
    ...
    priority_confidence = prob_high if pred_priority == 'High' else 1 - prob_high
    closure_confidence = prob_closure if pred_closure else 1 - prob_closure

    return {
        'predicted_priority': pred_priority,
        'priority_confidence': round(float(priority_confidence), 3),
        'predicted_road_closure': pred_closure,
        'closure_probability': round(float(prob_closure), 3),
        'closure_confidence': round(float(closure_confidence), 3),
        'predicted_duration_hours': round(float(pred_dur_hrs), 2),
        'estimated_severity_score': int(est_severity),
        'risk_level': risk,
        ...
        'nearest_hotspot_km': {k: round(float(event[f'dist_{k}_km']), 2) for k in HOTSPOTS},
    }
